        ...
        <div className="card">
            ...
            {merchantTab === 'REGISTER' && (
                <form onSubmit={handleRegisterShop}>
                    <h2>Opret Webshop 🛍️</h2>
                    <p className="text-muted">Få din egen portal på [ditnavn].returnwiz.dk</p>
                    <div className="form-group"><label>Shop Navn (uden mellemrum)</label><input value={shopName} onChange={e => setShopName(e.target.value)} placeholder="myshop" required /></div>
                    <div className="form-group"><label>Admin Email</label><input type="email" value={shopEmail} onChange={e => setShopEmail(e.target.value)} placeholder="admin@myshop.dk" required /></div>
                    <button className="btn-primary" disabled={loading}>{loading ? '...' : 'Opret Shop'}</button>
                    {shopSuccess && (
                        <div className="success-box" style={{marginTop:'20px', fontSize: '14px', wordBreak: 'break-all'}}>
                            ✅ {shopSuccess}
                        </div>
                    )}
                </form>
